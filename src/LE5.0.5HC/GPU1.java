...
class GPU extends ManagedComponent implements ToolTipMaker
{
	...
	//direct=new File(".").toString();
	direct=new File(".").getAbsoluteFile().toString();
	...
	if (state.tickGPU(clk,trigger_option) && gpusel >= 1)
		...
		if ((!Addrv.isUnknown())&&(!Addrv.isErrorValue())) { gpuAddr=(long)Addrv.toIntValue(); }
	...
	int size = getNominalBounds().getWidth();
...
//loadGpuMemory(true,"c:\\sim\\gpupram3");
	...
	private class State implements ComponentData, Cloneable
	{
		...
		{
		return new State(new BufferedImage(256, 256, BufferedImage.TYPE_INT_RGB));
		}
	}
	...
	private class VideoMenu implements ActionListener, MenuExtender
	{
		...
		else if (src == GPUhelp) showHelp("index0.html");
	...
	try
		...
		if (ROM)
			...
			attrs.setAttr(ROM_FILE,src.getPath());
			}
...
*/
	/* This help system is a modified version of Kevin Walsh's new help system that replaced Logisim's */
	...
	try
		...
		{
		Errors.title("Error Displaying Help").show("Help data not found.", e);
		}
	}
	...
	try
		{
		//URI uri = new URL(url).toURI(); // URL<String> is deprecated in Java 20
		URI uri = URI.create(url);
		Desktop desktop = Desktop.isDesktopSupported() ? Desktop.getDesktop() : null;
		if (desktop != null && desktop.isSupported(Desktop.Action.BROWSE)) { desktop.browse(uri); return; }
		}
	catch (Exception e)
		{
		Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
		clipboard.setContents(new StringSelection(url), null);
		Errors.title("Error Displaying Help").show(String.format("The following URL has been copied to the clipboard,\npaste it into your browser to access help.\nThe help URL is: %s", url), e);
		return;
		}
	Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
	clipboard.setContents(new StringSelection(url), null);
	Errors.title("Error Displaying Help").show(String.format("The following URL has been copied to the clipboard,\npaste it into your browser to access help.\nThe help URL is: %s", url));
	}
	...
	catch (Exception e) { internal_err(req, e); }
	}

	static void handle(HttpExchange req)
	{
	// The req path will look like "/html/index.html"
	// This maps to jar resource "/doc/html/index.html"
	...
	InputStream is;
	if (rsrc.endsWith("/")) { is = GPU.class.getResourceAsStream(rsrc + "index.html"); }
	else
		{
		is = GPU.class.getResourceAsStream(rsrc + "/index.html");
		if (is == null) is = GPU.class.getResourceAsStream(rsrc);
		}
	if (is == null) { send_err(req, 404, "Not found"); return; }

	byte[] body;
	try { body = is.readAllBytes(); }
	catch (IOException e) { send_err(req, 500, "Failed to read help resource"); return; }
	...
	if (warned) return;
	warned = true;
	SwingUtilities.invokeLater(() -> Errors.title("Error Displaying Help").show("Help data not found.", e));
	}
	...
	String url = "http://" + srv.getAddress() + "/html/" + target;
	openInBrowser(url);
	}
